...
const buildJavascriptHref = (functionName: string, ...args: string[]): string => (
  `javascript:${functionName}(${args
    .map((arg) => `decodeURIComponent('${encodeURIComponent(String(arg ?? ''))}')`)
    .join(',')});`
);

const joinEscapedLines = (values: unknown[] = []): string => values.map(escapeHtml).join('<br>');
...
export function renderLoadBalancer(loadbalancer: string, type: string) : string {
  ...
  if (type === 'display') {
    return `<a href="https://${escapeHtml(loadbalancer)}" target="_blank" class="plainLink">${
      escapeHtml(balancer)
      }</a>`;
  }
  return escapeHtml(balancer);
}

function renderVirtualServer(loadbalancer, name, type) {
  const vsName = escapeHtml(name.replace(/^\/Common\//, ''));
  const escapedLoadBalancer = escapeHtml(loadbalancer);
...
function renderRule(loadbalancer: string, name: string, type: string) {
  const ruleName = escapeHtml(name.replace(/^\/Common\//, ''));
  const escapedLoadBalancer = escapeHtml(loadbalancer);
  const escapedName = escapeHtml(name);
...
function renderPolicy(loadbalancer: string, name: string, type: string) {
  ...
  let result = '';
  const polName = escapeHtml(name.replace(/^\/Common\//, ''));
  const escapedLoadBalancer = escapeHtml(loadbalancer);
  const escapedName = escapeHtml(name);
...
function renderPool(loadbalancer: string, name: string, type: string) {
  ...
  const poolName = escapeHtml(name.replace(/^\/Common\//, ''));
  const escapedLoadBalancer = escapeHtml(loadbalancer);
  const escapedName = escapeHtml(name);
...
function renderCertificate(loadbalancer: string, name: string, type: string) {
  let result = escapeHtml(name.replace(/^\/Common\//, ''));
  const escapedLoadBalancer = escapeHtml(loadbalancer);
...
function renderDataGroup(loadbalancer: string, name: string, type: string) {
  const datagroupName = escapeHtml(name.replace(/^\/Common\//, ''));
  const escapedLoadBalancer = escapeHtml(loadbalancer);
  const escapedName = escapeHtml(name);
...
function showVirtualServerDetails(virtualserver: string, loadbalancer: string) {
  ...
  if (matchingVirtualServer) {
    ...
    const escapedPort = escapeHtml(port);
    const escapedProfileType = escapeHtml(profiletype);
    const escapedTrafficGroup = escapeHtml(trafficgroup || 'N/A');
    const escapedDescription = escapeHtml(description || '');
    const escapedCompressionProfile = escapeHtml(compressionprofile);
    ...
    table += `<td>
                <table class="virtualserverdetailstable">
                  ...
                  <tr><th>Source Translation</th><td>${escapeHtml(xlate)}</td></tr>
                  <tr>
      ...
      if (irules.length > 0) {
        ...
        irules.forEach(iRuleName => {
          ...
          if (siteData.preferences.ShowiRuleLinks) {
              ...
              if (iRule.datagroups && iRule.datagroups.length > 0) {
                iRule.datagroups.forEach((datagroup) => {
                  ...
                  if (siteData.preferences.ShowDataGroupLinks) {
                    ...
                    datagroupdata.push(escapeHtml(dataGroupName));
                  }
                });
              } else {
            ...
            table += `        <tr><td>${escapeHtml(iRuleName)}</td></tr>`;
          }
        });
...
function showiRuleDetails(name: string, loadbalancer: string) {
  ...
  if (matchingirule) {
    ...
    html += `<span>iRule: ${escapeHtml(matchingirule.name)}</span><br>`;
    html +=
      `<span>Load Balancer: ${
      renderLoadBalancer(loadbalancer, 'display')
      }</span>`;
...
function showPolicyDetails(policy: string, loadbalancer: string) {
  ...
  if (matchingPolicy) {
    ...
    html = `<div class="policydetailsheader">
               <span>Policy: ${escapeHtml(matchingPolicy.name)} </span>
               <br>
...
function showDataGroupDetails(datagroup, loadbalancer) {
  ...
  if (matchingDatagroup) {
    ...
    html += `<span>Data group: ${escapeHtml(matchingDatagroup.name)}</span><br>`;
    html +=
      `<span>Load Balancer: ${
      renderLoadBalancer(loadbalancer, 'display')
      }</span><br>`;
    html += `<span class="dgtype">Type: ${escapeHtml(matchingDatagroup.type)}</span>`;
    html += '</div>';
            ...
            render (data, type) {
              ...
              data.split(' ').forEach((item) => {
                if (type === 'display' && item.match(/^http(s)?:/)) {
                  result.push(`<a href="${escapeHtml(item)}">${escapeHtml(item)}</a>`);
                } else {
                  ...
                  if (pool) {
                    ...
                    result.push(type === 'display' ? escapeHtml(item) : item);
                  }
                }
              });
              return result.join(' ');
